...
heroes.forEach(hero => {
    ...
    if (savedRating) {
        document.getElementById(`${hero.name}-${savedRating}`).checked = true;
    }

    // Save rating to localStorage
    const ratingElements = document.querySelectorAll(`input[name="${hero.name}"]`);
    ratingElements.forEach(element => {
        element.addEventListener('change', (event) => {
            localStorage.setItem(hero.name, event.target.value);
        });
    });
});
